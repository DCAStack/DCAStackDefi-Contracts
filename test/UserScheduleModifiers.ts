...
import { ETH, DAI, DAI_CHECKSUM} from "./FaucetHelpers";
...
describe("UserScheduleFactory Test Suite", function () {
  ...
  expect(getSchedules.length).to.equal(0);
    ...
    hardhatUserScheduleModifiers.connect(addr1).depositFunds(ETH, ethers.utils.parseEther("7"), {value: ethers.utils.parseEther("7")});
    hardhatUserScheduleModifiers.connect(addr1).depositGas({value: ethers.utils.parseEther("1")});
  ...
  expect(getSchedules.length).to.equal(1);
    ...
    await expect(hardhatUserScheduleModifiers.connect(addr1).changeStatus(2, false)).to.be.reverted;
  ...
  expect(getSchedules.length).to.equal(1);
    ...
    await expect(hardhatUserScheduleModifiers.connect(addr3).changeStatus(0, false)).to.be.reverted;
...
it("Should delete schedule multiple", async function () {
    ...
    await expect(
        ...
        .to.emit(hardhatUserScheduleModifiers, "NewUserSchedule")
        .withArgs(i+1, DAI_CHECKSUM, ETH, addr1.address);
...
it("Should delete schedule invalid", async function () {
  ...
  await expect(hardhatUserScheduleModifiers.connect(addr1).deleteSchedule(1)).to.be.reverted;
...
it("Should NOT delete schedule multiuser", async function () {
  ...
  await expect(hardhatUserScheduleModifiers.connect(addr3).deleteSchedule(0)).to.be.reverted;
